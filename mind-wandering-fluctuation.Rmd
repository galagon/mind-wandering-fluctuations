...
This R markdown provides the basis for our manuscript, "Mind wandering rates fluctuate across the day: Evidence from an experience sampling study" (Smith, Mills, Paxton, & Christoff, *submitted*, *Cognitive Research: Principles and Implications*).
...
In order to conduct the analyses on the reanalyzed dataset (`dataold`) using only the data collected on weekends, set eval=TRUE. The study ran from September 15th to October 14th, 2016, with the weekend dates being the 17th, 18th, 24th, and 25th of September and the 1st, 2nd, 8th, and 9th of October. See Appendix C.
...
In order to conduct the analyses on the original data (`data`) with each probe categorized according to how many hours of the day the person had been awake upon answering, set `eval=TRUE`. See Appendix B.
...
# Generate Figure 2 - means and SE lines for FMT ratings at each hour of the day, and a red line to represent predictions of the cubic model
s = seq(8,24,2)
...
ggplot() + 
  ...
  ylab("Freedom of Movement in Thought") +
  xlab(xl) +
...
ggplot() + 
  ...
  stat_summary(aes(y=ScaledScore,
               ...
               data=data.agg) +
  theme_bw(base_size=10) + 
  ylab("Freedom of Movement in Thought") + xlab("Hour of the Day") +
...
ggplot() + 
  ...
  theme_bw(base_size=10) + ylab("Freedom of Movement in Thought") +xlab("Hour of the Day") +
...
ggplot() +
  ...
  theme_bw(base_size=10) + ylab("Task Unrelatedness of Thought") +xlab("Hour of the Day") +
  scale_x_continuous(breaks=seq(8,24,2)) + coord_cartesian(ylim = c(-.4,.3)) +
  theme(text = element_text(size=15))
...
ggplot() + 
  ...
  theme_bw(base_size=10) + ylab("Stimulus-Independence of Thought") +xlab("Hour of the Day") +
  scale_x_continuous(breaks=seq(8,24,2)) + coord_cartesian(ylim = c(-.4,.3)) +
  theme(text = element_text(size=15))
```
...
The first section of code (set to 'eval=FALSE' by default) reorganizes the levels of `Dimension` so that TUT is input first, allowing for a comparison of the TUT and SIT dimensions. See Supplementary Materials.
...
Fits flat, linear, and quadratic models to the data as well, compare model fit amongst them hiearchically, and generates graphs with red (cubic), orange (quadratic), yellow (linear), and green (flat)  lines representing predictions of different models. See Supplementary Materials.
...
ggplot() +
  ...
  ylab("Task Unrelatedness of Thought") +
  xlab("Hour of the Day") + scale_x_continuous(breaks=seq(8,24,2)) + 
  coord_cartesian(ylim = c(-.4,.3)) +
  theme(text = element_text(size=15))
...
ggplot() + 
  ...
  ylab("Stimulus-Independence of Thought") +
  xlab("Hour of the Day") + scale_x_continuous(breaks=seq(8,24,2)) + 
  coord_cartesian(ylim = c(-.4,.3)) +
  theme(text = element_text(size=15))
```
